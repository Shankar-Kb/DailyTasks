...
request.onload = function(){
    ...
    buttonFirst.addEventListener('click', ()=>{displayList(data, rows, 1); currentPage=1;});

    var buttonLast = createHtmlElement('button', 'button-last');
    buttonLast.innerHTML = 'Last';
    buttonLast.addEventListener('click', ()=>{displayList(data, rows, pageCount); currentPage=pageCount;});
    ...
    var buttonNext = createHtmlElement('button', 'button-next');
    buttonNext.innerHTML = 'Next';
    buttonNext.addEventListener('click', ()=>{displayList(data, rows, currentPage+1); if(currentPage<pageCount) currentPage++;});
    pageBox.append(buttonFirst, buttonLast, buttonPrevious, buttonNext);
